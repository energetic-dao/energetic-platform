      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center truncate">
          ...
          >
            ...
            <span>{{ isConnected ? wallet.account : 'Connect Wallet' }}</span>
          </button>
        </div>
      </div>
    </div>
  </nav>
  <wallet-connect-modal v-model:show="showWalletConnect" />
</template>
...
const { wallet, isConnected } = storeToRefs(walletStore);

const showWalletConnect = ref<boolean>(false);

const handleWallet = () => {
  // @ts-ignore
  walletStore.initialize(window?.kadena);
